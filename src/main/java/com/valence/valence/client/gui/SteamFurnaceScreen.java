...
public class SteamFurnaceScreen extends AbstractContainerScreen<SteamFurnaceMenu> {
    ...
    public SteamFurnaceScreen(SteamFurnaceMenu menu, Inventory inv, Component title) {
    ...
    @Override protected void renderBg(GuiGraphics gg, float delta, int mx, int my) {
        ...
        gg.fill(arrowX, arrowY, arrowX + arrowW, arrowY + arrowH, ARROW_BG);
        int fill = menu.getMaxProgress() > 0 ? menu.getProgress() * (arrowW - 2) / menu.getMaxProgress() : 0;
        if (fill > 0) gg.fill(arrowX + 1, arrowY + 1, arrowX + 1 + fill, arrowY + arrowH - 1, ARROW_FILL);
        gg.fill(arrowX + arrowW, arrowY + 3, arrowX + arrowW + 4, arrowY + arrowH - 3, ARROW_BG);
        gg.fill(arrowX + arrowW + 4, arrowY + 5, arrowX + arrowW + 6, arrowY + arrowH - 5, ARROW_BG);
